            ...
            <div class="footer__reactions reactions">
                              ...
                              <div class="modal fade" id="demoModal-{{$post->id}}" tabindex="-1" role="dialog" aria-labelledby="demoModalLabel" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                  <div class="modal-content">
                                    ...
                                    <div class="modal-body d-flex flex-column" style="font-size: 16px;">
                                      @if($x == 0)
                                        Donors haven't donate in this post yet !
                                      @else
                                      <div class="d-flex justify-content-around">
                                        <div style="font-size:18px;font-weight: bold;margin-left: 5rem;">Donor</div>
                                        <div style="font-size:18px;font-weight: bold;margin-left: 7rem;">Donation Time</div>
                                        <div style="font-size:18px;font-weight: bold;margin-left: 2rem;">Donation Type</div>
                                      </div>
                                      @foreach($donates as $donate)
                                        @foreach($users as $user)
                                           ...
                                           <div class="p-3 d-flex">
                                              <div class="header__left">
                                                ...
                                                @else
                                                  <a href="{{ url('/choosenDonerprofile', $user->id) }}"><img src="{{asset('images/user.png')}}" class="post__author-pic"/></a>
                                                @endif
                                              </div>
                                              <div class="mt-4">
                                            ...
                                            <div class="d-flex flex-column justify-content-around" style="font-size:16px;">
                                              {{$donate->created_at}}
                                            </div>
                                            <div class="d-flex flex-column justify-content-around">
                                              @foreach($orders as $order)
                                                @if($donate->post_id == $order->post_id && $donate->user_id == $order->user_id)
                                                  @php
                                                    $y = 1;
                                                    break;
                                                  @endphp
              ...
              <div class="footer__buttons buttons">
                <span class="buttons__like like">
                  @if($post->disable == 0)
                    <form method="post" action="{{ route('disablePost', $post->id) }}">
                     @csrf
                     <button class="button1" type="submit">  Disable  </button>
                    </form>
                  @elseif($post->disable == 1)
                    <form method="post" action="{{ route('avaliblePost', $post->id) }}">
                     @csrf
                     <button class="button1" style="color:#fff;" type="submit">  Make Available </button>
                    </form>
                  @endif
                </span>
              </div>
